...
logactfinder <- function(itarget, irxn, icond, conditions)
{
  ...
  logact_searchintvl = c(-10, 5)
  rt <- uniroot(modelfun, logact_searchintvl, extendInt = "yes")
...
equilibriumfinder <- function(itgts, irxn, icond, conditions)
{
  ...
  modelfun <- function(logain)
  {
    ...
    logact_guess[ifixed] <- logaeq[icond,ifixed]  # set O2, CO2, CO activities to those already solved
  ...
  rts <- multiroot(modelfun, guessin)
